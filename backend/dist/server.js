...
// Database connection pool with proper typing
...
app.get("/api/auth/verify", (0, cors_1.default)(corsOptions), async (req, res) => {
    try {
        const token = req.headers.authorization?.split(" ")[1] || req.cookies?.token;
        const deviceId = req.headers["x-device-id"];
        if (!token) {
...
app.post("/api/devices/register", authenticateToken, async (req, res) => {
    try {
        ...
        if (!userId) {
            console.error('Missing userId in auth token');
        ...
        if (!deviceId || !platform) {
            console.error('Missing required fields:', { deviceId, platform });
            return res.status(400).json({
                error: "Missing required fields",
                details: { required: ['deviceId', 'platform'], received: { deviceId, platform } }
            });
        }
    ...
    `);
        ...
        const [existingDevice] = await pool.query('SELECT id FROM devices WHERE user_id = ? AND device_id = ?', [userId, deviceId]);
        if (existingDevice.length) {
            // Update existing device
            await pool.query(`UPDATE devices 
         SET device_name = ?, platform = ?, is_active = TRUE, last_active = CURRENT_TIMESTAMP
         WHERE id = ?`, [deviceName || `${platform} Device`, platform, existingDevice[0].id]);
        }
        else {
            // Register new device
            await pool.query(`INSERT INTO devices (user_id, device_id, device_name, platform)
         VALUES (?, ?, ?, ?)`, [userId, deviceId, deviceName || `${platform} Device`, platform]);
        }
        res.json({
            success: true,
...
app.post('/api/apps/:appId/icon', authenticateToken, uploadMiddleware_1.upload.single('icon'), async (req, res) => {
    const conn = await pool.getConnection();
    try {
        ...
        const filename = (0, uploadMiddleware_1.generateUniqueFilename)(req.file.originalname);
        const iconUrl = await s3Service_1.S3Service.uploadIcon(req.file, filename);
        // Get the old icon URL if it exists
        const [oldIcon] = await conn.query('SELECT icon_url FROM apps WHERE id = ?', [appId]);
        // Update the app with new icon URL
        await conn.query('UPDATE apps SET icon_url = ? WHERE id = ?', [iconUrl, appId]);
        // If there was an old icon, delete it from S3
        if (oldIcon[0]?.icon_url?.includes(process.env.CDN_DOMAIN)) {
            await s3Service_1.S3Service.deleteIcon(oldIcon[0].icon_url);
        }
        res.json({ iconUrl });
    }
    catch (error) {
...
app.use((req, res, next) => {
    console.log('Request reached debug middleware:', {
        method: req.method,
        path: req.path,
...
app.get('/api/subscription/current', authenticateToken, async (req, res) => {
    try {
        const subscription = await subscriptionService_1.SubscriptionService.getUserSubscription(req.user.userId);
...
app.post('/api/subscription/create', authenticateToken, async (req, res) => {
    try {
        ...
        if (!planId || !paymentMethodId) {
            return res.status(400).json({
                error: 'Missing required fields: planId and paymentMethodId'
            });
        }
        console.log('Creating subscription:', {
            userId: req.user.userId,
            planId,
            paymentMethodId: paymentMethodId.substring(0, 10) + '...' // Log partial ID for security
        });
        const subscription = await subscriptionService_1.SubscriptionService.createSubscription(req.user.userId, planId, paymentMethodId);
...
app.post('/api/subscription/cancel', authenticateToken, async (req, res) => {
    try {
        ...
        const subscription = await subscriptionService_1.SubscriptionService.cancelSubscription(req.user.userId, cancelImmediately);
        res.json(subscription);
    }
    catch (error) {
...
// IMPORTANT: This must be before any other body parser middleware
app.post('/api/webhook/stripe', express_1.default.raw({ type: 'application/json' }), stripeWebhook_1.handleStripeWebhook);
// Other middleware should come after
app.use(express_1.default.json());
app.use(express_1.default.urlencoded({ extended: true }));
// Start server
app.listen(PORT, () => {
